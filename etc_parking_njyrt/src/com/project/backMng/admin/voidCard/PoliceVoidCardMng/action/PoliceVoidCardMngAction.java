package com.project.backMng.admin.voidCard.PoliceVoidCardMng.action;
...
@Controller
@RequestMapping(value = "backMng/admin/voidCard/PoliceVoidCardMng", method = { RequestMethod.GET, RequestMethod.POST })
public class PoliceVoidCardMngAction extends BaseController{

	private final static Logger log=LogManager.getLogger(PoliceVoidCardMngAction.class);
	/**
	 * service
	 */
	...
	public ModelAndView list(HttpServletRequest request){
		...
		ModelAndView view=new ModelAndView("/jsp/backMng/admin/voidCard/PoliceVoidCardMng/PoliceVoidCardMng_list.jsp");
		view.addObject(LIST, policeVoidCardMngService.findList(queryParam, page));
	...
	public ModelAndView add(HttpServletRequest request){
		ModelAndView view=new ModelAndView("/jsp/backMng/admin/voidCard/PoliceVoidCardMng/PoliceVoidCardMng_add.jsp");
		view.addObject("parks", policeVoidCardMngService.findParkList());
		return view;
	}
	/**
	 * 保存用户的信息
	...
	public String save(HttpServletRequest request,PoliceVoidCardBean bean){
		try {
			policeVoidCardMngService.save(bean,null);
	...
	public ModelAndView edit(HttpServletRequest request) throws UnsupportedEncodingException{
		...
		PoliceVoidCardBean bean=policeVoidCardMngService.findInfo(mv_license, plate_color);
		ModelAndView view=new ModelAndView("/jsp/backMng/admin/voidCard/PoliceVoidCardMng/PoliceVoidCardMng_edit.jsp");
	...
	public ModelAndView view(HttpServletRequest request) throws UnsupportedEncodingException{
		...
		ModelAndView view=new ModelAndView("/jsp/backMng/admin/voidCard/PoliceVoidCardMng/PoliceVoidCardMng_view.jsp");
		view.addObject(BEAN, bean);
	...
	public String update(HttpServletRequest request,PoliceVoidCardBean bean){
		try {
			policeVoidCardMngService.update(bean,null);
			return super.responseJsonText(ResultBean.newSuccessResult());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			log.error(e);
...
//		String id=new String(getStr(request,"ID").getBytes("iso8859-1"),"utf-8");
		try{
			...
			policeVoidCardMngService.delete(map,null);
			return super.responseJsonText(ResultBean.newSuccessResult());
		}catch (Exception e) {
	...
	public ModelAndView downLoad(HttpServletRequest request){
		...
		queryParam.putRequestString(request, "query_mv_license");
		queryParam.putRequestString(request, "query_b_list_type");
		List<PoliceVoidCardBean> list = policeVoidCardMngService.findList(queryParam, null);
		SimpleDateFormat sf=new SimpleDateFormat("yyMMdd");
		String targetFile="公安黑名单"+sf.format(new Date());
		...
		try {
			...
			for(PoliceVoidCardBean bean:list){
	...
	public String upload(HttpServletRequest request,MultipartFile file){
		if(file!=null){
			...
			try {
				List<PoliceVoidCardBean> list = daoruFileList(file.getInputStream());
				LoginUserBean user=(LoginUserBean) request.getSession().getAttribute("loginUser");
				for(PoliceVoidCardBean bean:list){
					System.err.println(bean.getb_list_type());
					...
					if(policeVoidCardMngService.findInfo(bean.getmv_license(),bean.getPlate_color())!=null){
	...
	public  synchronized List<PoliceVoidCardBean> daoruFileList(InputStream inputStream) throws IOException{
...
//		int rowSize = 0;
		...
		POIFSFileSystem fs = new POIFSFileSystem(inputStream);
	       ...
	       List<PoliceVoidCardBean> list=new ArrayList<PoliceVoidCardBean>();
