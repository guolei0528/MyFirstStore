				...
				<div class="box mt10">
					<div class="box-header">
						...
						<div class="col-sm-12 ">
							...
							<tr>
							<td>缴费车道：${queryParam.query_lane_name }   日期：${queryParam.query_date }</td>
									<td>
									...
									<input type="hidden" id="park_id"
										name="park_id" value="${queryParam.query_park_id}"/>
									<input type="hidden" id="area_id"
										name="area_id"  value="${queryParam.query_area_id}"/>
									<input type="hidden" id="lane_id"
										name="lane_id"  value="${queryParam.query_lane_id}"/>
									<input type="hidden" id="lane_name"
										name="lane_name" value="${queryParam.query_lane_name}"/>
									<input type="hidden" id="flow_type"
										name="flow_type" value="${queryParam.flow_type}"/>
									<input type="hidden" id="operator"
										name="operator" value="${queryParam.query_operator}"/>
									<input type="hidden" id="operator_name"
										name="operator_name" value="${queryParam.query_operator_name}"/>
									<input type="hidden" id="date"
										name="date" value="${queryParam.query_date}"/>
									<input type="hidden" id="query_lane_id"
										name="query_lane_id" value="${param.query_lane_id}"/>
									<input type="hidden" id="query_operator"
										name="query_operator" value="${param.query_operator}"/>
									<input type="hidden" id="query_statistics_date_from"
										name="query_statistics_date_from" value="${param.query_statistics_date_from}"/>
									<input type="hidden" id="query_statistics_date_to"
										name="query_statistics_date_to" value="${param.query_statistics_date_to}"/>
								</tr>
							</table>
						</div>
					</div>
...
<%-- 										<td>${obj.area_name}</td> --%>
										...
										<td>
										<c:choose>
												<c:when test="${obj.paymethod == 0 && obj.pdiscounttoll == obj.totaltoll && obj.pdiscounttoll > 0}">  
													全额付款
												</c:when>
												<c:when test="${obj.paymethod == 0 && obj.pdiscounttoll != obj.totaltoll && obj.pdiscounttoll > 0}">  
													优惠付款
												...
												<c:when test="${obj.paymethod == 9 || obj.paymethod == 10}">  
													人工放行
												</c:when>
												<c:when test="${obj.paymethod == 0 && obj.pdiscounttoll == 0}">  
													0元放行
