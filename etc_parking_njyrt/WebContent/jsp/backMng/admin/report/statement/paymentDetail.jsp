...
<body class="hold-transition skin-blue sidebar-mini">
	...
	<div id="mainContentDiv" style="margin-left: 180px">
		...
		<!-- Main content -->
		<form action="paymentDetail.shtml" method="post" id="pageForm"
			enctype="multipart/form-data">
			<div class="container" style="width: 100%">

				<div class="box mt10">
					<div class="box-header">
						<div class="col-sm-6">对账明细报表</div>
						<div class="col-sm-12 ">
...
<!-- 									<td>车牌号</td> 
									...
									<td>出口车道</td>
									<td><select id="query_exit_lane" name="query_exit_lane"
										style="WIDTH: 100%">
									...
									<td>出口时间</td>
									<td><input type="text" id="query_exit_date"
										name="query_exit_start_date" value="${queryParam.query_exit_start_date}"
										onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd HH:mm:ss' })" style="WIDTH: 90%"></td>
									<td>-</td>
									<td><input type="text" id="query_end_date"
										name="query_exit_end_date" onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd HH:mm:ss' })"
										value="${queryParam.query_exit_end_date}" style="WIDTH: 90%"></td>
									<td>
									<td>
									<td>付费类型</td>
									...
									<td>收费员编号</td> 
									<td>
									<input type="text" id="query_exitoperator" name=query_exitoperator value="${queryParam.query_exitoperator}" placeholder="" autocomplete="off">
									</td>
									<td>
...
<!-- 									<td>入口时间</td> 
					...
					<div class="box-body">
						...
						&nbsp;&nbsp;&nbsp;&nbsp; <label style="font-weight:bolder">应收金额:<font
							size="5" color="#00AA00"><fmt:formatNumber type="number"
									value="${receivablesSum/100 }" pattern="0.00" maxFractionDigits="2" /></font>元
						</label>
						&nbsp;&nbsp;&nbsp;&nbsp; <label style="font-weight:bolder">实收金额:<font
							size="5" color="#00AA00"><fmt:formatNumber type="number"
...
<%-- 										<td>${obj.exitdate}</td> --%> 
										...
										<td><c:choose>
												<c:when
													test="${obj.vehicleclass == 1 || obj.vehicleclass == 2}">  
												     小车
												</c:when>
												<c:when
													test="${obj.vehicleclass == 3 || obj.vehicleclass == 4}">  
												     大车
												</c:when>
											</c:choose></td>
										<%-- 										<td>${obj.cardid}</td> --%>
										...
										<td>
											<%-- 										${obj.entrytime} --%> <fmt:formatDate
												pattern="yyyy-MM-dd  HH:mm:ss" value="${obj.entrytime}" />
										</td>
										<td>
											<%-- 										${obj.exittime} --%> <fmt:formatDate
												pattern="yyyy-MM-dd  HH:mm:ss" value="${obj.exittime}" />
										</td>
										<td>
											<%-- 										<fmt:formatDate var="nowStr" value="${obj.exittime}" pattern="yyyy/MM/dd HH:mm:ss"/> --%>
											<%-- 											<c:set var="interval" --%> <%-- 												value="${obj.exittime.time - obj.entrytime.time}" /> <s:property --%>
											...
											<c:set var="interval"
												value="${obj.exittime.time - obj.entrytime.time}" /> <c:if
												...
												<fmt:formatNumber
												value="${interval/1000%60 -0.5}" pattern="#0" />秒
										</td>
										<%-- 										<td><c:set var="interval" value="<fmt:formatDate var="nowStr" value="${obj.exittime}" pattern="yyyy/MM/dd HH:mm:ss"/>"/></td> --%>
...
// 			alert(${queryParam.dis_free});
			...
			tool.selOption("query_exit_lane", '${queryParam.query_exit_lane}');
// 			var query_dis_free = '${queryParam.dis_free}';
