				...
				<div class="box-body">
					...
					<div class="form-group">
						<label for="mv_license" class="col-sm-2 control-label">
							车辆牌照<font color="#FF0000;">(必填*)</font>
						</label>
						<div class="col-sm-4">
							<input id="mv_license" name="mv_license" onchange="doChangeFlag1();"
								value="${bean.mv_license}" type="text" class="form-control"
								autocomplete="off" />
							<input id="old_mv_license" name="old_mv_license"
								value="${bean.mv_license}" type="hidden" class="form-control"
								autocomplete="off"  disabled="disabled"/>
							<input id="status" name="status"
								value="${bean.status}" type="hidden" class="form-control"
								autocomplete="off"  disabled="disabled"/>	
...
<!-- 						<div class="form-group"> -->
						...
						<div class="col">
							<label class="mt5 mr5"><input name="color" value="0" type="radio" autocomplete="off" checked="checked" />蓝</label>
						    <label class="mt5 mr5"><input name="color" value="1" type="radio" autocomplete="off" />黄</label>
						     <label class="mt5 mr5"><input name="color" value="2" type="radio" autocomplete="off" />黑</label>
						     <label class="mt5 mr5"><input name="color" value="3" type="radio" autocomplete="off" />白</label> 
						     <label class="mt5 mr5"><input name="color" value="4" type="radio" autocomplete="off" />绿</label>
					...
					<div class="form-group">
						<label for="vehicle_class" class="col-sm-2 control-label"> 车辆类型</label>
						<div class="col-sm-10">
							<label class="mt5 mr5"><input name="vehicle_class" value="1"
								type="radio" autocomplete="off" checked="checked" />小客车</label> <label
								class="mt5 mr5"><input name="vehicle_class" value="2"
								type="radio" autocomplete="off"  />大客车</label> <label class="mt5 mr5">
								<input name="vehicle_class" value="3" type="radio" autocomplete="off" />货车
							</label>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<input id="start_time" name="start_time" type="text" value="<fmt:formatDate value="${bean.valid_start_time}"
												type="date" pattern='yyyy-MM-dd HH:mm'/>"
								class="form-control"
								autocomplete="off" onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'end_time\',{d:0})}',dateFmt: 'yyyy-MM-dd HH:mm' })" />
						</div>
						<label for="end_time" class="col-sm-2 control-label">结束时间</label>
						<div class="col-sm-4">
							<input id="end_time" name="end_time" type="text" value="<fmt:formatDate value="${bean.valid_end_time}"
												type="date" pattern='yyyy-MM-dd HH:mm'/>"
								class="form-control"
								autocomplete="off" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'start_time\',{d:0})}',dateFmt: 'yyyy-MM-dd HH:mm' })" />
						</div>
					</div>
					
					<div class="form-group">
						<label for="toll_type" class="col-sm-2 control-label"> 收费形式</label>
						<div class="col-sm-4">
							...
							<!-- 记录计费 -->
							<input id="maketoll_type" name="maketoll_type" 
								value="${bean.toll_type}" type="text"   hidden="hidden;"/>
...
<!-- 								<option value="31">次数收费A</option> -->
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<input id="user_number" name="user_number"  onchange="doChangeFlag();"
								...
								autocomplete="off" />
							<input id="old_user_number" name="old_user_number"
								value="${bean.user_number}" type="hidden" class="form-control"
								autocomplete="off"  disabled="disabled"/>	
							<label style="cursor: pointer; color: white; background-color: green;"
						...
						<div class="col-sm-4">
							<input id="user_name" name="user_name"
								value="${bean.user_name}" type="text" class="form-control"
								autocomplete="off" />
						</div>
					...
					<div class="form-group">
						...
						<label for="spare1" class="col-sm-2 control-label"> 可拥有车位数</label>
						<div class="col-sm-4">
							<input id="spare1" name="spare1" value="${bean.spare1}"
								type="text" class="form-control" autocomplete="off" />
							<input id="old_spare1" name="old_spare1"
								value="${bean.spare1}" type="hidden" class="form-control"
								autocomplete="off"  disabled="disabled"/>
						</div>
						...
						<!-- 记录性别 -->
						<input id="makegender" name="makegender" 
								value="${bean.gender}" type="text"   hidden="hidden;"/>		
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<input id="phone" name="phone"  
								value="${bean.phone}" type="text" class="form-control"
								autocomplete="off" />
						</div>
				...
				<%--原来修改可以修改校区，已注释 	<div class="form-group">
...
<script type="text/javascript">
	...
	var old_toll_type = ${bean.toll_type};
	...
	function updateit(){
		...
		var vehicle_class =  $("input[name='vehicle_class']:checked").val();
		...
		var toll_type = $("input[name='toll_type']:checked").val();
		...
		var old_user_number = $("#old_user_number").val();
		var old_spare1 = $("#old_spare1").val();
...
/* 		var area_idstr=nowareadid;
			...
			if (result) {
				$.ajax({
					...
					data : {
						...
						'old_toll_type' :old_toll_type,
						'old_user_number' : old_user_number,
						'old_spare1' : old_spare1
	...
	$("#allArea").change(function() { 
		...
		tool.selRadio('color','${bean.color}');
...
// 			alert(dept_id);
	...
	//检查输入的车主工号是否已经存在  目的：是统一车主可拥有车位数
