...
<body class="hold-transition skin-blue sidebar-mini">
	...
	<div id="mainContentDiv" style="margin-left: 180px">
		...
		<!-- Main content -->
		<form action="timeoutCar.shtml" method="post" id="pageForm">
		<div class="container" style="width: 100%">
			...
			<div class="box mt10">
							...
							<tr>
								...
								<td>
									<select id="query_type">
										<option></option>
										<option value="0" selected="selected">超时</option>
										<option value="1">超额</option>
									</select>
								</td>
								<td>时间(分钟)</td>
								<td><input name="query_time" type="number" value="${queryParam.query_time }"></td>
							</tr>
						</table>
					</div>
				</div>
				<!-- /.box-header -->
				<div class="box-body">
					<table id="dataTable" class="table table-bordered table-striped table-condensed" style="font-size: 10px">
						<thead>
							<tr>
								<th>记录编号</th>
<!-- 								<th>卡网络编号</th> -->
...
<%-- 									<td>${obj.entryshift}</td> --%>
									...
									<td><a href="javascript:deleteData('${obj.mvlicense}','${obj.car_color}')"><i class="fa fa-trash-o text-red"></i><span class="text-red"></span>异常</a></td>
								</tr>
							</c:forEach>
						</tbody>
						<!-- 
...
<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
  <div class="modal-dialog modal-lg" role="document">
	...
	function explore(obj){
		...
		function deleteData(mvlicense,car_color){
			tool.confirm("确实要删除该异常条目吗？",function(result){
				if(result){
					$.ajax({
						type:'post',
						url:'delete.shtml',
						data:{"mv_license":mvlicense,
						      "car_color":car_color},
						dataType:'json',
...
// 								tool.alert("删除成功！");
		...
		$(function(){
			$("#query_type").change(function(){
				if($("#query_type").val()==1){
					$("#pageForm").attr("action","excessiveCar.shtml");
					tool.formSubmit();
				}
			});
