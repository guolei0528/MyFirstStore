		...
		<form class="form-horizontal">
             <div class="box-body">
				...
				<div class="form-group">
					<label for="area_id" class="col-sm-3 control-label">片区编号<oak:required></oak:required></label>
					<div class="col-sm-9">
...
// 			var log_address=$("#log_address").val();
			...
			if(!vcheck.isNumber(area_id)){
				tool.alert("编号应该为大于0的整数");
				return
			}
			if(space_number<0){
				tool.alert("请核实停车位数量");
...
// 			}
			tool.confirm('确实要提交吗？',function(result){
				 if(result){
					...
					$.ajax({
						type : 'post',
						url : '${contentPath }/backMng/platuser/sys/AreaInfoMng/save.shtml',
						data:{
							'area_id':area_id,
							'park_id':park_id,
							'area_name':area_name,
...
// 							'log_address':log_address,
						...
						success : function(data, textStatus) {
							if(data.success){
								tool.closeDialog();
								$("#pageForm").attr("action",'list.shtml?park_id='+park_id);
								tool.formSubmit();
							}else{
